...
(function(global, factory) {
	...
	! function() {
		...
		var e = function(e, t) {
			...
			return n.initMouseEvent(e, t.bubbles, t.cancelable, window, 0, t.screenX || 0, t.screenY ||
				0, t.clientX || 0, t.clientY || 0, t.ctrlKey || !1, t.altKey || !1, t.shiftKey || !
				1, t.metaKey || !1, t.button || 0, t.relatedTarget || null), n
		};
	...
	! function() {
		...
		"function" != typeof window.CustomEvent && (t.prototype = window.Event.prototype, window
			.CustomEvent = t);
	}();
				...
				xhr.onload = function() {
					...
					if (isHttpStatusSuccess) {
						...
						if (dataType === 'json') {
							try {
								responseData =
									method === 'HEAD' ? undefined : JSON.parse(xhr
									.responseText);
								eventParams.data = responseData;
							} catch (err) {
								textStatus = 'parsererror';
								trigger(ajaxEvents.ajaxError, eventParams, 'error', xhr,
									textStatus);
								reject(new Error(textStatus));
							}
							if (textStatus !== 'parsererror') {
								trigger(ajaxEvents.ajaxSuccess, eventParams, 'success',
		...
		$.fn[name] = function() {
			...
			return this.each(function(index, element) {
				...
				each(targets, function(_, target) {
					...
					$target[nameIndex ? 'insertAfter' : 'insertBefore'](
					element);
				});
			});
		};
	...
	each(['attr', 'prop', 'css'], function(nameIndex, name) {
		...
		$.fn[name] = function(key, value) {
			...
			return this.each(function(i, element) {
				set(element, key, isFunction(value) ? value.call(element, i, get(
					element, key)) : value);
			});
		};
	});
	...
	each(['Width', 'Height'], function(_, name) {
		each([("inner" + name), name.toLowerCase(), ("outer" + name)], function(funcIndex,
		funcName) {
			$.fn[funcName] = function(margin, value) {
				...
				// 设置每个元素的值
				return this.each(function(index, element) {
					return set(element, index, name, funcIndex, includeMargin,
						margin);
				});
			};
		});
		...
		this.each(function(_, element) {
			...
			$(elements).each(function(_, element) {
				...
				if (nodeName !== 'fieldset' &&
					element.name &&
					!element.disabled && ['input', 'select', 'textarea', 'keygen']
					.indexOf(nodeName) > -1 && ['submit', 'button', 'image', 'reset',
						'file'
					].indexOf(type) === -1 &&
	...
	Drawer.prototype.swipeSupport = function swipeSupport() {
		...
		function setPosition(translateX) {
			...
			var transformCSS = "translate(" + (-1 * rtlTranslateMultiplier * translateX) +
				"px, 0) !important;";
			var transitionCSS = 'initial !important;';
			that.$element.css('cssText', ("transform: " + transformCSS + "; transition: " +
				transitionCSS + ";"));
		}
		...
		if ((_a = options.buttons) === null || _a === void 0 ? void 0 : _a.length) {
			...
			each(options.buttons, function(_, button) {
				buttonsHTML +=
					...
					"class=\"mdui-btn mdui-ripple mdui-text-color-primary " + (button.bold ?
						'mdui-btn-bold' : '') + "\">" + (button.text) + "</a>";
			});
			buttonsHTML += '</div>';
		}
		// Dialog 的 HTML
	...
	mdui.prompt = function(label, title, onConfirm, onCancel, options) {
		...
		var content = '<div class="mdui-textfield">' +
			...
			(options.type === 'text' ?
				("<input class=\"mdui-textfield-input\" type=\"text\" value=\"" + (options
					.defaultValue) + "\" " + (options.maxlength ? 'maxlength="' + options.maxlength +
						'"' : '') + "/>") :
				'') +
			(options.type === 'textarea' ?
				("<textarea class=\"mdui-textfield-input\" " + (options.maxlength ? 'maxlength="' +
					options.maxlength + '"' : '') + ">" + (options.defaultValue) + "</textarea>") :
				'') +
			'</div>';
	...
	var Tooltip = function Tooltip(selector, options) {
		...
		extend(this.options, options);
		// 创建 Tooltip HTML
		this.$element = $(("<div class=\"mdui-tooltip\" id=\"" + ($.guid()) + "\">" + (this.options
		...
		this.$element = $('<div class="mdui-snackbar">' +
			...
			(this.options.buttonText ?
				("<a href=\"javascript:void(0)\" class=\"mdui-snackbar-action mdui-btn mdui-ripple mdui-ripple-white " +
					buttonColorClass + "\" " + buttonColorStyle + ">" + (this.options.buttonText) +
					"</a>") :
				'') +
	...
	$(function() {
		...
		$document.on('click', '.mdui-bottom-nav>a', function() {
			...
			$bottomNav.children('a').each(function(index, item) {
				...
				if (isThis) {
					componentEvent('change', 'bottomNav', $bottomNav[0],
					undefined, {
						index: index,
					});
				}
